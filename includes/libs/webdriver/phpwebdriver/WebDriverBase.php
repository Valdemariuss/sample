...
class WebDriverBase
{
    ...
    function __construct($_seleniumUrl, $browser)
    {
    ...
    protected function &curlInit($url)
    {
        ...
        #curl_setopt( $this->_curl, CURLOPT_HTTPHEADER, array("Connection: keep-alive", "Content-type: application/json;charset=UTF-8"));
        curl_setopt($this->_curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($this->_curl, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($this->_curl, CURLOPT_HEADER, false);
        #curl_setopt( $this->_curl, CURLOPT_FORBID_REUSE, true );
//		print_r($url."\n");
        return $this->_curl;
    }
    ...
    {
        $status = WebDriverResponseStatus::getResponseStatus($json_response);
        switch ($status) {
    ...
    {
        // PATCH
        if ($locatorStrategy == LocatorStrategy::id) {
            $locatorStrategy = LocatorStrategy::cssSelector;
            $value = "#{$value}";
        }
        if ($locatorStrategy == LocatorStrategy::className) {
            $locatorStrategy = LocatorStrategy::cssSelector;
            $value = ".{$value}";
        }
