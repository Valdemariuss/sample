...
class WebElement extends WebDriverBase
{
    ...
    function __construct($parent, $element, $options)
    {
        ...
        $this->reference = $element->{$element_id};
        $this->id = $element_id;
        ...
        parent::__construct($root . "/element/" . $this->reference, $parent->browser);
    }


    public function toElement()
    {
        return array($this->{'id'} => $this->{'reference'});
    }
    ...
    public function getAttribute($attribute)
    {
        ...
        if ($this->browser == 'firefox') {
            if ($attribute == 'innerHTML') return $this->getProperty('innerHTML');
            if ($attribute == 'value') return $this->getProperty('value');
        }
    ...
    {
        $request = $this->requestURL . '/property/' . $property;
        $response = $this->execute_rest_request_GET($request);
        $attributeValue = $this->extractValueFromJsonResponse($response);
        return ($attributeValue);
    }
