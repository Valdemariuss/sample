...
{
    function __construct($host, $port, $session = "", $browser)
    {
        parent::__construct("http://" . $host . ":" . $port . "/wd/hub"
            . ($session != "" ? "/session/$session" : ""), $browser);
    }

    /**
     * Connects to Selenium server.
     *
     * @param $capabilities        array with capabilities see: http://code.google.com/p/selenium/wiki/JsonWireProtocol#/session
     */
    public function connect($capabilities = array())
    {
        $request = $this->requestURL . "/session";
        $session = $this->curlInit($request);

        $postargs = json_encode($capabilities);
        ...
        while (count($response)) {
            if ($jsonResponse = json_decode(array_pop($response))) {
                break;
            }
        }
        ...
        if ($this->browser == 'chrome') {
            ...
            $dir = $jsonResponse->{'value'}->{'chrome'}->{'userDataDir'};
        } else {
            $sessionId = $jsonResponse->{'value'}->{'sessionId'};
            $dir = $jsonResponse->{'value'}->{'capabilities'}->{'moz:profile'};
        }

        $this->requestURL = $header['url'] . '/' . $sessionId;
        return array($sessionId, $dir);
    }
    ...
    {
        $response = $this->execute_rest_request_GET($this->requestURL
            . "/element/active");
    ...
    public function setWindowSize($w, $h, $windowName = "current")
    {
        $request = $this->requestURL . "/window/$windowName/size";
        $session = $this->curlInit($request);
    ...
    public function getWindowSize($windowName = "current")
    {
        $request = $this->requestURL . "/window/$windowName/size";
        $response = $this->execute_rest_request_GET($request);
        return $this->extractValueFromJsonResponse($response);
    ...
    public function setWindowPosition($x, $y, $windowName = "current")
    {
        $request = $this->requestURL . "/window/$windowName/position";
        $session = $this->curlInit($request);
        $args = array('x' => $x, 'y' => $y);
        $jsonData = json_encode($args);
        $this->preparePOST($session, $jsonData);
    ...
    public function updateLastAlive()
    {
        #return $this->setSessionStorageKey('last_alive_at', microtime(true));
        return WebDriver__UpdateLastAlive();
    }

    public function getLastAlive()
    {
        return WebDriver__GetLastAlive();
    }

    public function isBusy()
    {
        #return $this->getSessionStorageKey('is_busy');
        return WebDriver__IsBusy();
    }

    public function setIsBusy($s = true)
    {
        #return $this->setSessionStorageKey('is_busy', $s);
        WebDriver__SetIsBusy($s);
    }

    public function waitForIdle(
        $wait = WEBDRIVER__DEFAULTS__MAX_WAIT,
        $timeout = WEBDRIVER__DEFAULTS__TIMEOUT_FOR_BUSY
    ) {
        return WebDriver__WaitForIdle($this, $wait, $timeout);
    }

    public function getSessionStorageKeysList()
    {
        $request = $this->requestURL . "/session_storage";
        $response = $this->execute_rest_request_GET($request);
        return $this->extractValueFromJsonResponse($response);
    }
    ...
    {
        $request = $this->requestURL . "/session_storage/key/$key";
        $response = $this->execute_rest_request_GET($request);
        return $this->extractValueFromJsonResponse($response);
    }

    public function setSessionStorageKey($key, $value)
    {
        $request = $this->requestURL . "/session_storage";
        $session = $this->curlInit($request);
        $args = array('key' => $key, 'value' => __("%s", $value));
        $jsonData = json_encode($args);
        $this->preparePOST($session, $jsonData);
        $response = curl_exec($session);
        return $this->extractValueFromJsonResponse($response);
    ...
    {
        $response = $this->execute_rest_request_GET($this->requestURL . "/timeouts");
        return $this->extractValueFromJsonResponse($response);
    }


    public function setTimeouts($type, $ms)
    {
        $request = $this->requestURL . "/timeouts";
        $session = $this->curlInit($request);
        if ($this->browser == 'chrome') {
            if ($type == 'pageLoad') $type = 'page load';

            $args = array('type' => $type, 'ms' => $ms);
        } else {
            $args = array($type => $ms);
        }
        $jsonData = json_encode($args);
    ...
    {
        $request = $this->requestURL . "/alert/text";
        $session = $this->curlInit($request);
    ...
    {
        $request = $this->requestURL . "/alert/accept";
        $session = $this->curlInit($request);
        $this->preparePOST($session, '{}');
        $response = curl_exec($session);
        return $this->extractValueFromJsonResponse($response);
    ...
    {
        $request = $this->requestURL . "/alert/dismiss";
        $session = $this->curlInit($request);
        $this->preparePOST($session, '{}');
        $response = curl_exec($session);
        return $this->extractValueFromJsonResponse($response);
     ...
     */
    public function execute($script, $script_args = array())
    {
        $request = $this->requestURL . "/execute/sync";
        $session = $this->curlInit($request);
    ...
    public function execute_async($script, $script_args = array())
    {
        $request = $this->requestURL . "/execute/async";
        $session = $this->curlInit($request);
        $args = array('script' => $script, 'args' => $script_args);
        $jsonData = json_encode($args);
        $this->preparePOST($session, $jsonData);
    ...
    {
        $request = $this->requestURL . "/window/handles";
        $response = $this->execute_rest_request_GET($request);
        return $this->extractValueFromJsonResponse($response);
    }
    ...
    public function moveTo($element = null, $xoffset = null, $yoffset = null)
    {
        ...
        if ($xoffset) {
            $args['xoffset'] = intval($xoffset);
        }
        if ($yoffset) {
            $args['yoffset'] = intval($yoffset);
        }
        if (empty($args)) {
            $this->preparePOST($session, '{}');
        } else {
    ...
    {
        $request = $this->requestURL . "/window/maximize";
        $session = $this->curlInit($request);
        $this->preparePOST($session, '{}');
        curl_exec($session);
    ...
    public function zipToBase64($filepath)
    {
