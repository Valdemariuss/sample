...
class WebDriverResponseStatus
{
    ...
    static function getResponseStatus($response)
    {
        ...
        if (!is_object($value) || !property_exists($value, 'error')) {
            return WebDriverResponseStatus::Success;
        }

        $error = $value->{'error'};
        // TODO
        if (WebDriverResponseStatus::matchError($error, 'no such element')) {
            return WebDriverResponseStatus::NoSuchElement;
        }

        return WebDriverResponseStatus::UnknownError;
    }

    private static function matchError($error, $text)
    {
        return strpos($error, $text) !== false;
    }
}
